#!/usr/bin/env python3
"""Offline, standard-library installer for a fingerprinted Armored Core 4 resource release."""
import argparse
import hashlib
import json
import os
...
def check_game(root, m):
    ...
    if meta.get('TITLE_ID') != m['game'] or meta.get('VERSION') != m['version'] or meta.get('APP_VER'):
        raise ValueError('仅支持日版 BLJM60012 / VERSION 01.05；所选游戏版本不符。')
...
def load(package):
    ...
    m = json.loads(raw)
    if m['format'] != 2 or m['game'] != 'BLJM60012':
...
def plan(package, m, roots):
    ...
    for r in m['unchanged']:
        ...
        if not p.is_file() or p.stat().st_size != r['size'] or digest(p) != r['sha256']:
            raise ValueError('原版资源不匹配（其他补丁、加密或不完整文件）：' + r['path'])
    selected = [('base', r) for r in m['base']]
    if 'update' in roots:
        ur = roots['update']
        meta = sfo(safe(ur, 'PARAM.SFO'))
        if (meta.get('TITLE_ID') != m['game'] or meta.get('VERSION') != m['version']
                or meta.get('CATEGORY') != 'GD' or meta.get('APP_VER')):
            raise ValueError('更新目录版本不符，仅支持已登记的 BLJM60012 更新资源。')
        for p in safe(ur, 'USRDIR').iterdir():
            if p.name != 'regulation.bin':
                raise ValueError('更新目录存在未登记的覆盖资源：' + p.name)
        for r in m['update_unchanged']:
            p = safe(ur, r['path'])
            if not p.is_file() or p.stat().st_size != r['size'] or digest(p) != r['sha256']:
                raise ValueError('更新目录存在未适配的资源版本：' + r['path'])
        selected.extend(('update', r) for r in m['update'])
    changes = []
    already = 0
...
def apply(package, m, roots, changes):
    ...
    backup_parent = safe(roots['base'], 'AC4_CN_Backups')
    backup = backup_parent / ident
    affected = {c['scope'] for c in changes}
    stages = {s: safe(roots[s], '.ac4-cn-stage-' + ident) for s in affected}
...
def restore(backup, overrides=None, check_process=True):
    ...
    for root in roots.values():
        ...
        if lock.exists() and json.loads(lock.read_text(encoding='utf-8'))['id'] == journal['id']:
            lock.unlink()
        stage = safe(root, '.ac4-cn-stage-' + journal['id'])
        if stage.exists():
            shutil.rmtree(stage)
...
def main():
    parser = argparse.ArgumentParser(description='装甲核心 4 独立汉化安装器（RPCS3 / 日版 BLJM60012）')
    parser.add_argument('--game', type=Path)
    parser.add_argument('--update', type=Path, help='可选：RPCS3 的 dev_hdd0/game/BLJM60012')
    ...
    if not any((args.game, args.restore)):
        ...
        if mode == '3':
            ...
            if input('是否同时汉化已经安装的 AC4 更新数据？输入 y，或直接回车仅安装本体：').strip().lower() == 'y':
                selected = choose('选择 RPCS3 的 dev_hdd0/game/BLJM60012 文件夹')
                if not selected:
                    return
                args.update = Path(selected)
        else:
            return
    roots = {}
    if args.game:
    ...
    if args.update:
        roots['update'] = args.update.expanduser().resolve()
    if args.restore:
